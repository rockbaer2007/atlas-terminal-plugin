...
Switch the interface between German and English. Use the slider to set a font size from 11 to 26 px; the choice is saved locally in the browser. The terminal prefers `MesloLGM Nerd Font Mono` when it is installed on the computer running the browser, then falls back to other available monospace fonts. When the ATLAS host has Oh My Posh and Bash installed, select a theme from the dropdown for local terminal sessions. The theme list comes from the UGSo fork and the selection is saved in this browser. SSH sessions use the remote server's own shell and do not apply a local theme. ANSI colors are rendered by xterm.js.
